...
public class SuperMissile extends Missile{
	...
	public SuperMissile(int x, int y, Direction dir) {
		super(x,y,dir);
		this.x = x;
		this.y = y;
		this.dir = dir;
	...
	public SuperMissile(int x,int y,boolean good,Direction dir,TankClient tc,int level) {
		this(x,y,dir);
		this.good=good;
		this.tc=tc;
		this.level=level;
	...
	public boolean hitMissile(Missile m) {
		if(m.isGood()!=this.good&&m.isLive()&&this.live&&this.getRech().intersects(m.getRech())) {
			if(m.getClass().equals(DeathMissile2.class)||m.getClass().equals(LighteningMissile.class)) {
				live=false;
			}
			else {
				live = true;
